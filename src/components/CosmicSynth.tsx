...
export default function CosmicSynth() {
  ...
  function initAudio() {
    try {
      ...
      const ld=new Tone.PolySynth(Tone.FMSynth,{maxPolyphony:6,options:{harmonicity:2.5,modulationIndex:4,oscillator:{type:"fatsawtooth",spread:15,count:3},modulation:{type:"triangle"},envelope:{attack:.05,decay:.3,sustain:.5,release:1.5},modulationEnvelope:{attack:.08,decay:.2,sustain:.4,release:1}}});
      ld.volume.value=-10;
      const sb=new Tone.PolySynth(Tone.Synth,{maxPolyphony:6,options:{oscillator:{type:"sine"},envelope:{attack:.08,decay:.2,sustain:.6,release:1.2}}});
      sb.volume.value=-16;
      ...
      const pd=new Tone.PolySynth(Tone.Synth,{maxPolyphony:6,options:{oscillator:{type:"fatsine4" as any,spread:30,count:4},envelope:{attack:2.5,decay:1,sustain:.7,release:3}}});
      ...
      const bs=new Tone.PolySynth(Tone.Synth,{maxPolyphony:3,options:{oscillator:{type:"fatsquare",spread:8,count:2},envelope:{attack:.02,decay:.15,sustain:.7,release:.5}}});
      ...
      const ar=new Tone.PolySynth(Tone.Synth,{maxPolyphony:4,options:{oscillator:{type:"triangle"},envelope:{attack:.01,decay:.1,sustain:.2,release:.4}}});
      ...
      const dn=new Tone.PolySynth(Tone.Synth,{maxPolyphony:4,options:{oscillator:{type:"sine"},envelope:{attack:3,decay:2,sustain:.8,release:5}}});
